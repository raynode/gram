...
const isList = (type) => isNonNullType(type) ? isList(type.ofType) : graphql_1.isListType(type);
...
exports.createFilterStrategy = (middlewares) => (inputType, inputName) => {
    ...
    return middlewares.reduce((result, [check, filter]) => check(type, isRequired, isListType, baseType)
        ? Object.assign({}, result, filter(name, type, list)) : result, {});
};
