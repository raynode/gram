...
export declare const fieldsReducer: <Context>(reducer: (contextModel: ContextModel<Context, any, any>) => GraphQLFieldConfigMap<any, any, {
    [key: string]: any;
}> | GraphQLInputFieldConfigMap) => (context: Wrapped<Context>) => FieldReducerFn<Context>;
export declare const reduceFields: <Context, Reducers, Models, ReducerKeys extends keyof Reducers>(models: Record<string, ModelBuilder<Context, any, any>>, reducers: Record<ReducerKeys, FieldReducerFn<Context>>, fields: Record<ReducerKeys, GraphQLFieldConfigMap<any, any, {
    [key: string]: any;
...
export declare const memoizeContextModel: <Context, Result, Type = any>(fn: (contextModel: ContextModel<Context, Type, Type>) => Result) => ((contextModel: ContextModel<Context, Type, Type>) => Result) & import("lodash").MemoizedFunction;
export declare const reduceContextFields: <Context, Type extends Record<string, any>>(contextModel: ContextModel<Context, Type, Type>, base: Type, reducer: (memo: Type, attr: AttributeBuilder<Context, Type, any>, type: GraphQLType, field: ModelType<Context>) => Type) => {};
...
export declare const createContextModelFieldFn: <Context>(configFn: (contextModel: ContextModel<Context, any, any>) => ContextModelFieldFnConfig) => (contextModel: ContextModel<Context, any, any>) => {
    subscribe: () => any;
    resolve: ({ node }: {
        node: any;
    }) => any;
...
export declare const createInputType: <Context>(field: DataType, nameFn: ContextModelFn<string>) => ((contextModel: ContextModel<unknown, any, any>) => GraphQLInputObjectType) & import("lodash").MemoizedFunction;
export {};
