...
const serviceToVisibility = (service) => ({
    ...
    findManyQuery: service.hasOwnProperty('findMany') &&
...
exports.createModelBuilder = (modelName, service, contextFn) => {
    ...
    const builder = {
        ...
        attr: (attributeName, type) => {
            ...
            const contextFn = typeof type === 'function'
                ? type
                : graphql_1.isType(type)
                    ? () => type
                    : context => context.getModel(type.name);
            return (attributes[attributeName] = attributeBuilder_1.createAttributeBuilder(attributeName, contextFn));
        },
        setup: context => {
